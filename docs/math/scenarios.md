...
### Same math: 3 × 8 TB RAID1

| | |
|--|--|
| Usable | $24/2 =$ **12 TB** |
| After one loss | $16/2 =$ **8 TB** |
| $\Delta_{\mathrm{fit}}$ | **4 TB** |
| Suggested Warning / Critical | **4 T** / **4 T** |

Contrast **2 × 8 TB RAID1**: after one loss the survivor still holds a full copy → $\Delta \approx 0$ (no soft capacity-fit floor). See [raid1.md](raid1.md).
...
|---------|----------------|
| $\Delta_{\mathrm{fit}}$, Warning / Critical rule | **Suggest free thresholds** on that pool’s tab (`warn = \max\Delta$, `crit = \min\Delta`; hidden for 2-disk RAID1) |
